...
from experimental.lazy_volume_level import runner as experimental_runner
import random
...
def run_single_task(experiment_config: ExperimentConfig) -> str:
    ...
    use_lazy_volume_runner = (
        experiment_config.task_config.task_name == "volume_level_decoding_task"
        and model_spec.constructor_name in {"brainbert_finetune", "popt_finetune"}
    )

    runner_fn = (
        experimental_runner.run_training_over_lags_lazy_stft
        if use_lazy_volume_runner
        else decoding_utils.run_training_over_lags
    )
    if use_lazy_volume_runner:
