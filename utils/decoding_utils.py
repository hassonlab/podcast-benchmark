    ...
    for fold, (tr_idx, va_idx, te_idx) in zip(fold_nums_all, fold_indices):
        ...
        # elif feature cache, overwrite loaders and model => not ideal but we wanna do it quickly for now. Clean up later.
            ...
            if model_name == "ReferenceBrainBERTDecoder":  # *brainbert
                ...
                cached_feature_head = _build_gpt2_cached_feature_head(
                    model.encoder_model
                )
...
def check_model_train_eval_and_requires_grads(
    ...
    print(
        f"Parameter tensors requiring grad: {num_params_requires_grad} out of {sum(1 for p in model.parameters())} total parameters"
    )
