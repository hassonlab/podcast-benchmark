...
usage() {
    cat <<'EOF'
Usage:
  ./commands/debug/submit-task.sh --model MODEL --task TASK [options] [-- EXTRA_MAIN_ARGS...]

This submitter is for debugging on the standard benchmark path.
...
It forwards to:
  ./commands/submit-task.sh

Examples:
  ./commands/debug/submit-task.sh --model brainbert --task content_noncontent --epochs 1
  ./commands/debug/submit-task.sh --model popt --task llm_decoding --node node1
EOF
}
...
exec "${PROJECT_ROOT}/commands/submit-task.sh" "${ARGS[@]}"
