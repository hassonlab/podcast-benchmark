...
usage() {
    cat <<'EOF'
Usage:
  ./commands/experimental/lazy_volume_level/submit-task.sh --model MODEL --task TASK [options] [-- EXTRA_MAIN_ARGS...]

This submitter is isolated from the standard path and uses:
...
CONFIG_REL_PATH="configs/experimental/lazy_volume_level/${MODEL}/${TASK}/${VARIANT}.yml"
...
mkdir -p "${PROJECT_ROOT}/logs/experimental/lazy_volume_level"

if [[ -z "$JOB_NAME" ]]; then
    JOB_NAME="pbexp-${MODEL}-${TASK}-${VARIANT}"
fi
...
SBATCH_ARGS=(
    ...
    --mem="${MEM_GB}G"
    --job-name="${JOB_NAME}"
    --output="${PROJECT_ROOT}/logs/experimental/lazy_volume_level/%x-%j.out"
    --error="${PROJECT_ROOT}/logs/experimental/lazy_volume_level/%x-%j.err"
)
...
echo "Submitting experimental lazy_volume_level path: ${MODEL}/${TASK}/${VARIANT}"
