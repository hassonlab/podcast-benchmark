...
class PopulationTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        if use_brainbert:
            ...
            try:
                from models.brainbert.simple_transformer import load_pretrained_model as load_brainbert
                ...
                for param in self.stft_preprocessor.parameters():
                    param.requires_grad = False
                print(f"Initialized STFT preprocessor with fs={sample_rate}, freq_channels=40")
                
            except Exception as e:
                print(f"Warning: Failed to load BrainBERT from {brainbert_model_dir}: {e}")
        ...
        if use_lip_coords:
            ...
            self.foundation_model.pos_encoder = MultiSubjBrainPositionalEncoding(
                self.foundation_model.model_dim,
                dropout,
                max_coord_value
            )
            # Move to device
        ...
        if self.use_brainbert:
            ...
            if self.foundation_model.input_channels != expected_input_channels:
                print(f"Reinitializing input_projection for BrainBERT output: "
                      f"{self.foundation_model.input_channels} -> {expected_input_channels}")
                model_dim = self.foundation_model.model_dim
                self.foundation_model.input_projection = nn.Linear(expected_input_channels, model_dim)
    ...
    def forward(self, x, **kwargs):
        ...
        if data_info_list is not None:
            ...
            lip_coords = torch.stack(coord_tensor, dim=0)  # [batch_size, num_channels, 3]
        ...
        if self.use_brainbert:
            ...
            if is_stft_preprocessed:
                ...
                with torch.no_grad():
                    ...
                    all_emb_seq = self.brainbert(x_brainbert, return_sequence=True)  # [batch*channels, time_stft, 768]
                ...
                if self.stft_preprocessor is None:
                    raise ValueError("Raw signal input detected but STFT preprocessor not initialized. "
                                   "Either enable use_stft_preprocessing in data_params or initialize STFT preprocessor.")
                
                batch_size, num_channels, time_steps = x.shape
                
                # Process all channels at once (matching original PopT implementation)
                x_reshaped = x.contiguous().view(batch_size * num_channels, time_steps)  # [batch*channels, time]
                
                with torch.no_grad():
                    # STFT preprocessing: [batch*channels, time] → [batch*channels, time_stft, 40]
                    all_spec = self.stft_preprocessor(x_reshaped)  # [batch*channels, time_stft, 40]
                    ...
                    all_spec_brainbert = all_spec.transpose(1, 2)  # [batch*channels, 40, time_stft]
                    
                    # BrainBERT inference (frozen, no gradients) - all channels at once
                    all_emb_seq = self.brainbert(all_spec_brainbert, return_sequence=True)  # [batch*channels, time_stft, 768]
            ...
            if self.use_lip_coords:
                ...
                features = self.foundation_model(x_emb, return_sequence=False, lip_coords=lip_coords)
            else:
            ...
            if self.use_lip_coords:
                if lip_coords is None:
                    raise ValueError("lip_coords is required when use_lip_coords=True")
                features = self.foundation_model(x, return_sequence=False, lip_coords=lip_coords)
            else:
...
def create_finetuning_decoder(model_params):
    ...
    return PopulationTransformerDecoder(
        ...
        use_brainbert=model_params.get("use_brainbert", False),  # Will be overridden by config setter
...
def set_finetuning_config(experiment_config, raws, _df_word):
    ...
    use_lip_coords = experiment_config.model_params.get("use_lip_coords", False)
    
    # If use_brainbert is not explicitly set, determine based on use_lip_coords
    if "use_brainbert" not in experiment_config.model_params:
        if use_lip_coords:
            # LIP coordinates require BrainBERT (original PopT implementation)
            experiment_config.model_params["use_brainbert"] = True
            print("use_lip_coords=True: automatically setting use_brainbert=True (original PopT with LIP)")
        else:
            # No LIP: use direct input processing (existing benchmark behavior)
            experiment_config.model_params["use_brainbert"] = False
            print("use_lip_coords=False: automatically setting use_brainbert=False (direct input processing)")
    else:
        # use_brainbert was explicitly set, use that value
        use_brainbert = experiment_config.model_params["use_brainbert"]
        if use_lip_coords and not use_brainbert:
            print("Warning: use_lip_coords=True but use_brainbert=False. "
                  "This may not match original PopT implementation.")
    
    use_brainbert = experiment_config.model_params.get("use_brainbert", False)
    ...
    if use_brainbert:
        ...
        print(f"STFT preprocessing enabled in data loading: fs={sample_rate}, freq_channels=40, nperseg=400, noverlap=350")
        
        # BrainBERT outputs 768-dimensional embeddings per channel
        # PopT will receive [batch, channels, 768] instead of [batch, channels, time]
        experiment_config.model_params["input_channels"] = 768
        ...
        print(f"Direct input processing: input_channels set to {experiment_config.model_params.get('input_channels', 'unknown')}")
