...
from registry import task_data_getter_registry
import task_utils  # Import to populate the registry
...
class TestTrainingMatrixConfigs:
    ...
    def test_training_matrix_structure(self, training_matrix):
        ...
        for model_name, tasks in training_matrix.items():
            if not isinstance(tasks, dict):
                continue
            assert isinstance(tasks, dict), f"Model '{model_name}' should map to a dict of tasks"
    ...
    def test_all_configs_are_valid_yaml(self, training_matrix, configs_dir):
        ...
        for model_name, task_name, config_path in config_paths:
            try:
                ...
                invalid_configs.append((model_name, task_name, str(config_path), str(e)))

        assert len(invalid_configs) == 0, \
            f"Invalid YAML configs: {invalid_configs}"
    ...
    def test_all_configs_load_as_experiment_config(self, training_matrix, configs_dir):
        ...
        for model_name, task_name, config_path in config_paths:
            try:
                ...
                failed_conversions.append((model_name, task_name, str(config_path), str(e)))
...
class TestTrainingMatrixTaskDataGetters:
    ...
    def test_all_tasks_are_registered(self, training_matrix):
        ...
        assert len(unregistered_tasks) == 0, \
    ...
    def test_all_config_task_names_match_matrix(self, training_matrix, configs_dir):
        ...
        for model_name, expected_task_name, config_path in config_paths:
            ...
            config_task_name = config_dict.get("task_name", "word_embedding_decoding_task")
    ...
    def test_all_task_data_getters_exist(self, training_matrix, configs_dir):
        ...
        for model_name, task_name, config_path in config_paths:
            ...
            try:
                ...
                # Verify it exists
                assert task_data_getter is not None, \
                    f"Task '{task_name}' not found in registry"

                # Verify it's callable
                assert callable(task_data_getter), \
                    f"Task '{task_name}' is registered but not callable"

            except Exception as e:
                missing_or_invalid_tasks.append((model_name, task_name, str(config_path), str(e)))

        assert len(missing_or_invalid_tasks) == 0, \
...
class TestTaskDataGetterRegistry:
    ...
    def test_registry_is_not_empty(self):
        ...
        assert len(task_data_getter_registry) > 0, \
            "Task data getter registry is empty"
    ...
    def test_required_tasks_are_registered(self):
        ...
        for task_name in expected_tasks:
            assert task_name in task_data_getter_registry, \
