...
`make submit-word-label-reruns` submits the canonical word-label task rerun matrix with model-specific lag chunking and collision-free trial names; its `DRY_RUN=1` mode prints the complete Slurm plan without submitting it.
