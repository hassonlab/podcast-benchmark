...
`configs/` contains experiment YAML files. `configs/foundation_models/` holds generated benchmark configs for foundation models and tasks. These configs wrap each ordered foundation preprocessing pipeline with `disk_cache_preprocessor` so extracted foundation features can be reused from `.cache/preprocessors/`. Disk preprocessor caches are canonical per pipeline/source/params, mode, lag, and subject channel metadata, and store row start times inside the cache so subset and superset jobs can reuse matching rows. Non-volume foundation configs update the canonical cache with missing rows; volume-level foundation configs leave existing caches unchanged and opt into temporary chunked preprocessing so large lag tensors are not fully materialized after preprocessing. BrainBERT and PopT configs declare STFT as an explicit wrapped preprocessing step before foundation feature extraction. Nested foundation specs inside `foundation_feature_cache` are cache-only feature extraction specs and intentionally omit task head parameters. `configs/examples/` contains smaller examples.
...
`Makefile`, `setup.sh`, and `submit.sh` provide setup and execution conveniences for local or cluster workflows.
