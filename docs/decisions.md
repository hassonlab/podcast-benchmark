...
Foundation model configs wrap their ordered preprocessing steps with the registered `disk_cache_preprocessor` so expensive foundation feature extraction can be reused from disk. They keep `config_setter_name: foundation_feature_cache` so nested foundation model settings are still prepared before training. Model-specific steps such as BrainBERT/PopT STFT are declared as explicit wrapped preprocessor entries with their own params instead of hidden `DataParams` flags.

The disk cache identity includes pipeline source, normalized params, mode, lag, and subject channel metadata, but not selected row start times. Each cache file stores normalized row starts next to the output data, which lets subset and superset jobs share one canonical file and reuse only matching rows in the requested order. Missing rows are computed by slicing `selected_rows` and `selected_rows_df` before running the wrapped pipeline. By default those computed rows are returned without mutating an existing cache; `update_cache_with_missing: true` atomically overwrites the same file with the merged row union. Non-volume foundation configs opt into that update behavior so feature caches fill in over time, while volume-level configs keep it disabled because those jobs should not write additional cache data during partial reuse.

This partial reuse design assumes wrapped preprocessing is row-wise across examples, which matches the intended foundation feature caching pipelines. Batch-stat preprocessors can produce different results when recomputed on only missing rows and should not be wrapped with `disk_cache_preprocessor`.
