...
The disk cache identity includes pipeline source, normalized params, mode, lag, and subject channel metadata, but not selected row start times. Each cache file stores normalized row starts next to the output data, which lets subset and superset jobs share one canonical file and reuse only matching rows in the requested order. Missing rows are computed by slicing `selected_rows` and `selected_rows_df` before running the wrapped pipeline. By default those computed rows are returned without mutating an existing cache; `update_cache_with_missing: true` atomically overwrites the same file with the merged row union.
